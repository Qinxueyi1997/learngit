...
void Get_Current_Body_Data(char *dat)
{
    ...
    cJSON_AddNumberToObject(root,"breathAmp",HTTP_Body_Data[1]);//呼吸频率
    cJSON_AddNumberToObject(root,"sleep",HTTP_Body_Data[2]);
    cJSON_AddNumberToObject(root,"bodyTP",HTTP_Body_Data[3]);//体温
    cJSON_AddStringToObject(root,"pushTime",cname);
  //  cJSON_AddStringToObject(root,"pushTime","2020-02-02 21:20:20");
     char *out=cJSON_PrintUnformatted(root);
     memcpy(dat,out,strlen(out));//将整合的数据copy给dat
     cJSON_Delete(root);
     free(out);
