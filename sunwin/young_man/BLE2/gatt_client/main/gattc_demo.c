...
void Get_CurrentData(char *dat)
{
    ...
    cJSON_AddStringToObject(root,"deviceId","SWC20200520000120210520");// 儿童
    cJSON_AddNumberToObject(root,"breathFlag",HTTP_Send_Data[0]);//呼吸频率
    cJSON_AddNumberToObject(root,"breathAmp",HTTP_Send_Data[1]);//呼吸频率
    cJSON_AddNumberToObject(root,"sleep",HTTP_Send_Data[2]);
...
static void peripheral_get(void *param)
{         if(sht30_get_value()==ESP_OK)   //获取温湿度
        {
         ...
         xTaskCreate(rx_task, "uart_rx_task", 1024*2, NULL, 11, NULL); 
...
/**************************sntp时间戳初始化结束************************************/
       ...
       if(Tmp_Hum_Time==10){
            Tmp_Hum_Time=0;
            xTaskCreate(&peripheral_get, "peripheral", 4096, NULL, 5, NULL);
           }
        }
    }
}
static void peripheral_init(void *param)
{   uart_init(); 
    i2c_init();                         //I2C初始化
    sht30_init();                       //sht30初始化
