...
#include "lwip/dns.h"
short http_send_flag=0,wifi_stop_flag=0,wifi_start_flag=0;
char wifi_flag=0;
	  ...
	  //err = esp_timer_start_periodic(fw_timer_handle, 1250*1000);//625us秒回调
        ...
        if (time1==80) {
           ...
           http_send_flag++;
            ...
            { wifi_stop_flag++;
              wifi_start_flag++;
            if(wifi_stop_flag==30)
            {   wifi_stop_flag=0;
               ESP_ERROR_CHECK( esp_wifi_stop() );
               }
                if(wifi_start_flag==60)
                {
               wifi_start_flag=0;
               app_wifi_initialise();//启动wifi

                }
               //  return_now = time(&now);
               // localtime_r(&now, &timeinfo);
                //strftime(strftime_buf, sizeof(strftime_buf), "%c", &timeinfo);
                 if((wifi_start_flag<30)&&(wifi_start_flag>0))
