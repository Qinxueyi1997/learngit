...
char *cname,post_data2[2048]={0},wifi_flag=0,http_send_fail=0,left_bed_flag=0,move_body_flag=0;
short sleep_post_flag=0,http_send_flag=0,low_power_flag=0,sntp_init_flag=0,http_data_type_flag=0;
int HTTP_Send_Data[20]={0},HTTP_Body_Data[6]={7,20,0,0,0,1},HTTP_MonitorSleep_Data[1]={3},current_hour=0,current_min=0;
...
{ 
    ...
    if (err == ESP_OK) { 
        ...
        xTaskCreate(&wifi_led_test_task, "led_test_task", 2048, NULL, 11, NULL);//数据发送不成功 ，灯快速闪动
            http_send_fail++;
            if(http_send_fail==5)//连续五次发送数据失败
            {
                http_send_fail=0;
                esp_restart();
            }
        ...
        //printf("Sleep after two minutes:\n");
            ...
            if(seg_pre_abssum < 200)
             {     
                    ...
                    HTTP_Body_Data[0]=7;
                    left_bed_flag++;
                    if(left_bed_flag>=6)left_bed_flag=6;
                    if(left_bed_flag<=5)
                    {      
             ...
             if(judge_whether_human(X_X,N_seg) == 1){
                ...
                if(not_people_counter >= 6 || people_counter <= 5) {
				  ...
				  if(pr_diff >= 2000){
                    ...
                    move_body_flag++;
                    if(move_body_flag>=6)move_body_flag=6;
                    if(move_body_flag<=5)
                     {          rem_move_t++;
                                http_data_type_flag=2;
                                xSemaphoreGive(adc_semaphore);
                ...
                if(count_stage >=240){
                        ...
                        printf("std_array_t[std_t]:%f       \n",std_array_t[std_t]);
                        printf("std_t:       %d\n",std_t);
                        if(std_t>=1)
                        {
                            for(int i=0;i<=std_t;i++)
                            {
                                printf("std_array_t:    %f   \n",std_array_t[i]);
                            }
                                   ...
                                   // if((std_array_t[std_t]-0.5<=std_array_t[std_t-1])&&(rem_leave_t<=3)&&(rem_move_t<=10))//与前一次标准差做比对
