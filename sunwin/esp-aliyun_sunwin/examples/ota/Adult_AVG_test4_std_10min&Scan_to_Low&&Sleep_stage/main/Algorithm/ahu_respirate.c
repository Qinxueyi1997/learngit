...
int judge_whether_human(double * array,int size)
{
    ...
    for(int i = 0; i <size ;i++){
        ...
        if(X_seg_demean[i] < 0){//取均值
            X_seg_demean[i] = -X_seg_demean[i];//取绝对值
        }
    }
...
int sleep_analysis(double * heart,int len,double norm_heart,double N_thr_value){
  ...
  //  if(fabs(norm_heart - mean_heart) <= 30 && fabs(norm_heart - mean_heart) >= 5 && std_heart < N_thr_value){
     if(fabs(norm_heart - mean_heart) <= 30 && fabs(norm_heart - mean_heart) >= 5){
        cur = 3;
    }
    else if(fabs(norm_heart - mean_heart) < 5 && fabs(norm_heart - mean_heart) >= 3){
        cur = 2;
    }
    else{
        cur = 1;
...
int sleep_up_analysis(double * heart,int len,double norm_heart,double N_thr_value){
    int cur = 1;
    double mean_heart = mean(heart,0,len);
    double std_heart = standard_deviation(heart,0,len);
  //  if(fabs(norm_heart - mean_heart) <= 30 && fabs(norm_heart - mean_heart) >= 5 && std_heart < N_thr_value){
