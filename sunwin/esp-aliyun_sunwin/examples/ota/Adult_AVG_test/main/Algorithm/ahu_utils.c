...
void sort_descend(double array[],int size) //闁跨喐甯存潏鐐鐏忓繘鈧鏁撻弬銈嗗闁跨喐鏋婚幏鐑芥晸閺傘倖瀚归柨鐔告灮閹风兘鏁撻弬銈嗗
{
    double temp;//闁跨喐鏋婚幏閿嬫闁跨喐鏋婚幏鐑芥晸閺傘倖瀚�
...
void sort_ascend(double array[],int size) //闁跨喐鏋婚幏宄扮毈闁跨喐鏋婚幏鐑芥晸閺傘倖瀚归柅澶愭晸閺傘倖瀚归柨鐔告灮閹风兘鏁撻弬銈嗗闁跨喐鏋婚幏鐑芥晸閺傘倖瀚�
{
    double temp;//闁跨喐鏋婚幏閿嬫闁跨喐鏋婚幏鐑芥晸閺傘倖瀚�
    for(int i = 0; i < size-1; i++)
    {
...
{
    int temp;//闁跨喐鏋婚幏閿嬫闁跨喐鏋婚幏鐑芥晸閺傘倖瀚�
    for(int i = 0; i < size-1; i++)
    {
        for(int j = size-1; j > i; j--)
        {
...
double standard_deviation(const double array[],int len1,int len2){
    double avg = mean(array,len1,len2);//濮瑰倸閽╅崸鍥р偓锟�
    double sum = 0.;
...
//matlab闁跨喐鏋婚幏绌宨ff闁跨喐鏋婚幏鐑芥晸閺傘倖瀚归柨鐔告灮閹凤拷
...
void diff_fabs(double *dat,int datalen,double*ret_buf,int *ret_len)
{
  ...
  for(i = 1; i<datalen; i++)
  {
    ret_buf[j++] = fabs(dat[i]-dat[i-1]);//姹傛诞鐐规暟缁濆鍊�
  }
  *ret_len = j;
}
...
/** 闁跨喐鏋婚幏鐑芥晸閺傘倖瀚归柨鐔烘幐eakdet闁跨喐鏋婚幏鐑芥晸閺傘倖瀚归柨鐔告灮閹峰嘲鈧ジ鏁撻弬銈嗗濮ｅ繘鏁撻弬銈嗗闁跨喐鏋婚幏宄扳偓濂告晸閺傘倖瀚规惔鏃堟晸閺傘倖瀚归柨鐔告灮閹风兘鏁撻弬銈嗗闁跨喕顫楃敮顔藉c闁跨喐鏋婚幏鐑芥晸閺傘倖瀚归柨鐔荤窛鐢喗瀚归幗娲晸閺傘倖瀚归柨鐔告灮閹风兘鏁撻弬銈嗗鐟曚線鏁撻惌顐ゎ暜閹风兘鏁撻弬銈嗗閸婂ジ鏁撻弬銈嗗闁跨喐鏋婚幏鐑芥晸閺傘倖瀚归柨鐔稿疆鐢喗瀚筸atlab娑撯偓闁跨喖鎽敐蹇斿闁跨喐鏋婚幏鐑芥晸閻偆顣幏閿嬫闁跨喐鏋婚幏閿嬫暈闁跨喐鏋婚幏鐑芥晸閿燂拷1*/
