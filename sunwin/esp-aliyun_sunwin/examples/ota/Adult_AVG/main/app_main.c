...
char *cname,post_data2[1024]={0},wifi_flag=0,http_send_fail=0;
    ...
    if(adc_semaphore!=NULL)
    {
        ...
        {  //printf("原始数据上传一次.............\n");
            ...
            Get_Current_Original_Data(post_data2);//得到json数据，用如post data
             vTaskDelay(5 / portTICK_PERIOD_MS);///freertos里面的延时函数
        }else if(http_data_type_flag==2){
         //  printf("体征数据上传一次..................\n");
            address = webaddress[1];
            Get_Current_Body_Data(post_data2);//得到json数据，用如post data
            vTaskDelay(5 / portTICK_PERIOD_MS);///freertos里面的延时函数
        }
        else if(http_data_type_flag==3){
         //  printf("睡眠信息上传一次..................\n");
