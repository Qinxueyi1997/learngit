...
char *cname,*month,post_data2[2048]={0},wifi_flag=0,http_send_fail=0,http_post=0,left_bed_flag=0,bed_touch_flag=0,move_body_flag=0;
...
TaskHandle_t ota_task_Handler,led_test_task_Handler,http_test_task_Handler,low_power_Handler,adc1_get_data_task_Handler,over_time_scan_task_Handler;
nvs_handle handle;
    ...
    //创建smartconfig任务
        ...
        if(wifi_flag==1)
        {
               if (s_retry_num < EXAMPLE_ESP_MAXIMUM_RETRY) {
                ...
                if(s_retry_num>=45)//link timeout ,will enter low power
                 xTaskCreate(&low_vol_protect_task, "low_vol_protect_task", 2048, NULL, 6,&low_power_Handler);  //创建低功耗任务
...
void app_main(void)
{
   ...
   //install gpio isr service
    ...
    xTaskCreate((void (*)(void *))ota_main, "ota_example", 20480, NULL, 6, &ota_task_Handler);
