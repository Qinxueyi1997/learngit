...
#define EXAMPLE_ESP_WIFI_SSID      "aithinker"
...
static void udp_client_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        while (1) {
            ...
            //angle_result=(int)(angle_pitch-angle_calculation);
           // peripheral_get_task();//获取温湿度数据
           // sprintf(post_data2,"TS%dHS%dAS%dES",environment[0],environment[1],angle_result);//组数据格式
            //printf("post_data2:%s\n",post_data2);
           ...
           memset(post_data2,0,strlen(post_data2));
            ...
            sprintf(post_data2,"hello");//组数据格式
             printf("post_data2:%s\n",post_data2);
            int err = sendto(sock, post_data2, strlen(post_data2), 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
           
           memset(post_data2,0,strlen(post_data2));
            sprintf(post_data2,"how are you ");//组数据格式
             printf("post_data2:%s\n",post_data2);

            vTaskDelay(2000 / portTICK_PERIOD_MS);
             err = sendto(sock, post_data2, strlen(post_data2), 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
           
           memset(post_data2,0,strlen(post_data2));
                      sprintf(post_data2,"cccccddddd");//组数据格式
                       printf("post_data2:%s\n",post_data2);
