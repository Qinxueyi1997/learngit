...
/* Generic OnOff Server message handlers发送数据的句柄 */
                                  ...
                                  bool publish)
{
    ...
    if (ctx == NULL && publish == false) {
        BT_ERR("%s, Invalid parameter", __func__);//打印错误警告
        return;
    }
...
static void gen_onoff_get(struct bt_mesh_model *model,
                          struct bt_mesh_msg_ctx *ctx,
                          struct net_buf_simple *buf)//接收开关命令
{
    struct bt_mesh_gen_onoff_srv *srv = model->user_data;//接收用户数据
    ...
    send_gen_onoff_status(model, ctx, false);//发送开关状态命令
    return;
}
...
static void gen_onoff_set(struct bt_mesh_model *model,
                          struct bt_mesh_msg_ctx *ctx,
                          struct net_buf_simple *buf)//状态设置命令
{
    struct bt_mesh_gen_onoff_srv *srv = model->user_data;
   ...
   /* if (srv->rsp_ctrl.set_auto_rsp == BLE_MESH_SERVER_RSP_BY_APP) {
    ...
    if (bt_mesh_is_server_recv_last_msg(&srv->last, tid, ctx->addr, ctx->recv_dst, &now)) {
        ...
        BT_ERR("%s, Invalid OnOff value 0x%02x", __func__, 0xf6);
        send_gen_onoff_status(model, NULL, true);
