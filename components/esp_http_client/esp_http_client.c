...
static esp_err_t http_dispatch_event(esp_http_client_t *client, esp_http_client_event_id_t event_id, void *data, int len)//分发事件函数
{
...
static int http_on_body(http_parser *parser, const char *at, size_t length)
{
    ...
    http_dispatch_event(client, HTTP_EVENT_ON_DATA, (void *)at, length);//分发一个HTTP_EVENT_ON_DATA的事件
    return 0;
}
...
{//开始http会话之前必须先调用这个初始化函数，
//他的返回值它的返回值esp_http_client_handle_t型变量作为其他HTTP API的实参用。
...
esp_err_t esp_http_client_set_url(esp_http_client_handle_t client, const char *url)//设置url
{
    char *old_host = NULL;
...
static int esp_http_client_get_data(esp_http_client_handle_t client)//这个函数是用来读取HTTP应答的消息体BODY数据的，读取数据并解析
{
...
int esp_http_client_read(esp_http_client_handle_t client, char *buffer, int len) //获取http返回的body数据,从数据流中读取数据
{
    esp_http_buffer_t *res_buffer = client->response->buffer;
...
esp_err_t esp_http_client_perform(esp_http_client_handle_t client)//完成tcp发送和接收
                                                              
{//注意case之间没有break,所以一下程序为顺序执行
    esp_err_t err;
    do {//第一部分
        ...
        /* In case of blocking esp_http_client_perform(), the following states will fall through one after the after;
           ...
           esp_http_client_perform API again, and for this reason, we maintain the states */
            case HTTP_STATE_INIT://http状态初始化
                ...
                /* falls through */
            case HTTP_STATE_CONNECTED://http建立链接
                ...
                /* falls through */
            case HTTP_STATE_REQ_COMPLETE_HEADER://http请求发送成功
                ...
                /* falls through */
            case HTTP_STATE_REQ_COMPLETE_DATA://http请求回应数据
                ...
                /* falls through */
            case HTTP_STATE_RES_COMPLETE_HEADER://检查http回应数据
...
//第三大部分
int esp_http_client_fetch_headers(esp_http_client_handle_t client)//等待接收数据
{//就是在里面一直等待http返回的应答数据，里面设计了返回应答数据，如果接收超时，就退出
    ...
    client->response->status_code = -1; //错误信息
...
esp_err_t esp_http_client_open(esp_http_client_handle_t client, int write_len)//链接tcp,并发送HTTp头信息
{
...
esp_err_t esp_http_client_set_post_field(esp_http_client_handle_t client, const char *data, int len)//设置url和解析参数
{//这个函数首先设置http请求的数据body,这个数据是第二步得到json数据，然后下面设计content_type的内容编码，我们这里使cjson，所以设置成application/json
    ...
    if (client->post_data) {
        ...
        if (value == NULL) {
           // err = esp_http_client_set_header(client, "Content-Type", "application/x-www-form-urlencoded");
            err = esp_http_client_set_header(client, "Content-Type", "application/json");//请求方式改为json请求，2020.3.4秦学义修改
...
int esp_http_client_get_status_code(esp_http_client_handle_t client)//判断http应答状态，应答信号是200
{
    return client->response->status_code;
}

int esp_http_client_get_content_length(esp_http_client_handle_t client)//得到body的数据长度
{
    return client->response->content_length;
}
