...
void app_main(void)
{
    ...
    double event_type_vec = 0;// 0 正常 1 低通气 2 睡眠暂停 3 呼吸过快 4 坠床 5 体动
    ...
    while(1) {
        ...
        if(data_index <= 1200){
            X_X[begin1] = read_raw1;
            begin1++;
            if(begin1==N_seg)
                begin1 = 0;
        }
        if((data_index <= 1200) && (data_index % N_seg ==0)){
            double * a1 = (double *)malloc(N_seg*sizeof(double));
            wavedec_wrcoef(X_X,a1,N_seg);
            median_energy_vec[adative_range_index] = accumulate1(a1,sizeof(a1)/sizeof(double),N_seg);
            rate_value_vec1 = obtain_resprate_by_spec(a1,sizeof(a1)/sizeof(double),N_seg);
            rate_value_vec2 = obtain_resprate_by_corr(a1,sizeof(a1)/sizeof(double));
        ...
        if((data_index > 1200)&&(data_index % N_seg==0)){
            ...
            if((tt - 12000) % 1000 == 0){
                printf("\n%llu second~",(data_index - 1200)/10-9);
                printf("%llu second: ",(data_index - 1200)/10);
                if(event_type_vec == 0)
                    printf("normal");
                if(event_type_vec == 1)
...
void detect_local_interval(const double array[],double a[],int size,int Nmin,int Nmax)
{
    ...
    for(int i = Nmin; i <= Nmax; i++)
    {
        ...
        corr_vec[i-Nmin] = accumulate2(seg_left, seg_right,sizeof(seg_left)/sizeof(double));
    }
...
{
    int Fs = 10;
    double pr[size],pi[size],fr[size],fi[size];
    ...
    int half_length = sizeof(pr)/sizeof(double)/2;
    double max_value_index[2];
