...
char oled_show_num(uint16_t x,uint16_t y, uint32_t num, FontDef_t* Font, SSD1306_COLOR_t color) 
{
  uint8_t t,temp;
  uint8_t enshow=0;
   oled_gotoXY(x,y);//定位开始位置
   for(t=0;t<2;t++)
   {temp=(num/mypow(10,2-t-1))%10;
   if(enshow==0&&t<1)
   {if(temp==0)
   {oled_show_char(x+( Font->FontWidth)*t,y,' ', Font, color);
   continue;
   }else enshow=1;
   }
   oled_show_char(x+( Font->FontWidth)*t,y,temp+'0', Font, color);
   }
    oled_update_screen();
	return num;
}